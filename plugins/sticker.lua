      ...
      if data[tostring(msg.to.id)] then
        ...
        elseif matches[1] == 'sticker' then
          if matches[2] == 'warn' then
            ...
            return 'ØªÙ… ØªÙØ¹ÙŠÙ„ Ø­Ø¸Ø± Ø§Ù„Ù…Ù„ØµÙ‚Ø§Øª.\n'
                   ..'Ø³ÙˆÙ ÙŠØªÙ… ØªÙ†Ø¨ÙŠÙ‡ Ø§ÙˆÙ„Ø§ Ø«Ù… Ø§Ù„Ø­Ø¸Ø± Ø¨Ù„Ø§Ù†ØªÙ‡Ø§Ùƒ Ø§Ù„Ø«Ø§Ù†ÙŠ.'
          elseif matches[2] == 'kick' then
            if settings.sticker ~= 'kick' then
              settings.sticker = 'kick'
              save_data(_config.moderation.data, data)
            end
            return 'ØªÙ… ØªÙØ¹ÙŠÙ„.\nÙ…Ù†Ø¹ Ø§Ù„Ù…Ù„ØµÙ‚Ø§Øª Ø¨Ù„Ø­Ø¸Ø±'
          elseif matches[2] == 'ok' then
            if settings.sticker == 'ok' then
              return 'Ø§Ù†Ù‡ Ù…Ø¹Ø·Ù„ Ø¨Ù„ÙØ¹Ù„'
            else
              settings.sticker = 'ok'
              save_data(_config.moderation.data, data)
              return 'ØªÙ… ØªØ¹Ø·ÙŠÙ„'
            end
          end
